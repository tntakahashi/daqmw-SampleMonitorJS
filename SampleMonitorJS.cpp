...
static const char* samplemonitorjs_spec[] =
{
    "implementation_id", "SampleMonitorJS",
    "type_name",         "SampleMonitorJS",
    "description",       "SampleMonitorJS component",
    "version",           "1.0",
...
SampleMonitorJS::SampleMonitorJS(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("samplemonitorjs_in",   m_in_data),
      ...
      m_debug(false)
{
    ...
    // Set InPort buffers
    registerInPort ("samplemonitorjs_in",  m_InPort);
...
SampleMonitorJS::~SampleMonitorJS()
{
}

RTC::ReturnCode_t SampleMonitorJS::onInitialize()
{
    if (m_debug) {
        std::cerr << "SampleMonitorJS::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t SampleMonitorJS::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int SampleMonitorJS::daq_configure()
{
    std::cerr << "*** SampleMonitorJS::configure" << std::endl;
...
int SampleMonitorJS::parse_params(::NVList* list)
{
    ...
    for (int i = 0; i < len; i+=2) {
        ...
        if (sname == "server") {
            if (m_debug) {
                std::cerr << "server : " << svalue << std::endl;
            }
            m_server_name = svalue;
        }
...
int SampleMonitorJS::daq_unconfigure()
{
    std::cerr << "*** SampleMonitorJS::unconfigure" << std::endl;
    if (m_canvas) {
        delete m_canvas;
        m_canvas = 0;
    }
...
int SampleMonitorJS::daq_start()
{
    std::cerr << "*** SampleMonitorJS::start" << std::endl;

    m_in_status  = BUF_SUCCESS;

    //////////////// HTTP server ///////////////////
    if (!m_server) {
      m_server = new THttpServer(m_server_name.c_str());
    }
...
int SampleMonitorJS::daq_stop()
{
    std::cerr << "*** SampleMonitorJS::stop" << std::endl;
...
int SampleMonitorJS::daq_pause()
{
    std::cerr << "*** SampleMonitorJS::pause" << std::endl;

    return 0;
}

int SampleMonitorJS::daq_resume()
{
    std::cerr << "*** SampleMonitorJS::resume" << std::endl;

    return 0;
}

int SampleMonitorJS::reset_InPort()
{
    int ret = true;
    while(ret == true) {
        ret = m_InPort.read();
...
int SampleMonitorJS::decode_data(const unsigned char* mydata)
{
...
int SampleMonitorJS::fill_data(const unsigned char* mydata, const int size)
{
...
unsigned int SampleMonitorJS::read_InPort()
{
...
int SampleMonitorJS::daq_run()
{
    if (m_debug) {
        std::cerr << "*** SampleMonitorJS::run" << std::endl;
    }
...
extern "C"
{
    void SampleMonitorJSInit(RTC::Manager* manager)
    {
        RTC::Properties profile(samplemonitorjs_spec);
        manager->registerFactory(profile,
                    RTC::Create<SampleMonitorJS>,
                    RTC::Delete<SampleMonitorJS>);
    }
};
